...
function renderReview(state) {
    ...
    const resultMeta = {
        applied: { color: A.green,  icon: '✓', label: 'APPLIED' },
        skipped: { color: A.yellow, icon: '⚠', label: 'SKIPPED' },
        failed:  { color: A.red,    icon: '✗', label: 'FAILED'  },
    };
    const rm = entry.result ? (resultMeta[entry.result] || { color: A.dim, icon: '·', label: entry.result.toUpperCase() }) : null;

    // Colored group title when result is set
    const titleColor = rm ? rm.color + A.bold : '';
    const titleReset = rm ? A.reset : '';
    lines.push(titleColor + clip(
        `  Group ${gi + 1}/${total}  ·  ${method}:${fp}  ·  ` +
        `${entry.files.length} files  ·  ${humanSize(grpSz)}`,
        w
    ) + titleReset);
    lines.push(sep(w));
    ...
    if (rm) {
        lines.push(rm.color + A.bold + `  ${rm.icon}  ${rm.label}` + A.reset);
        lines.push(sep(w));
    }
    ...
    for (let i = 0; i < entry.files.length; i++) {
        ...
        const ac     = rm ? A.dim : actionColor(f.action);
        const act    = ac + pad(f.action, 7) + A.reset;
        const sz     = rpad(humanSize(f.size), 8);
        const dt     = fmtDate(f.mtime);
        const nm     = clip(f.path, nameW);
        const rowDim = rm ? A.dim : '';
        const rowRst = rm ? A.reset : '';
        lines.push(`${rowDim}${cursor} ${act} ${sz}  ${pad(dt, 12)}  ${nm}${rowRst}`);
    }
    ...
    // ── Groups progress ─────────────────────────────────────────────────────
    const gp = { pending:0, applied:0, skipped:0, failed:0 };
    for (const g of groups)
        if (g.result) gp[g.result] = (gp[g.result] || 0) + 1;
        else gp.pending++;
    const done = total - gp.pending;

    // Each bar cell covers a proportional slice of groups; colored by the
    // highest-priority state in that slice (failed > skipped > pending > applied).
    // The current group's cell is rendered bright/bold so you can see where you are.
    const barW = Math.min(40, w - 40);
    const barCells = [];
    for (let i = 0; i < barW; i++) {
        const gStart   = Math.floor((i / barW) * total);
        const gEnd     = Math.max(gStart + 1, Math.floor(((i + 1) / barW) * total));
        const slice    = groups.slice(gStart, gEnd);
        const isCur    = gi >= gStart && gi < gEnd;
        const hasFail  = slice.some(g => g.result === 'failed');
        const hasSkip  = slice.some(g => g.result === 'skipped');
        const hasPend  = slice.some(g => !g.result);
        const color    = hasFail  ? A.red
                       : hasSkip  ? A.yellow
                       : hasPend  ? A.dim
                       :            A.green;
        const ch       = hasPend  ? '░' : '█';
        barCells.push(isCur
            ? `\x1b[1m${color}█${A.reset}`   // bold to make current position pop
            : `${color}${ch}${A.reset}`
        );
    }
    lines.push(
        `  ${A.dim}groups:${A.reset} ${barCells.join('')} ${done}/${total}` +
        (gp.applied ? `   ${A.green}✓ ${gp.applied} applied${A.reset}` : '') +
        (gp.skipped ? `   ${A.yellow}⚠ ${gp.skipped} skipped${A.reset}` : '') +
        (gp.failed  ? `   ${A.red}✗ ${gp.failed} failed${A.reset}`   : '') +
        (gp.pending ? `   ${A.dim}${gp.pending} pending${A.reset}`    : '')
    );
    ...
    lines.push(
        ...
        `   [s]ave  [S]ave-as  [a]pply  [R]escan` +
        (state.canAbort ? `  ${A.red}[A]${A.reset}bort` : '') +
        `  [q]uit`
    );
    lines.push(sep(w));
...
function runInteractive(state) {
    return new Promise((resolve, reject) => {
        ...
        // Snapshot the session file content before any writes so abort can restore it.
        // Only needed for user-provided (non-temp) files; temp files are just deleted.
        const fs = require('fs');
        const _sessionSnapshot = (!state.isTemp && state.sessionFile && fs.existsSync(state.sessionFile))
            ? fs.readFileSync(state.sessionFile, 'utf8')
            : null;

        // canAbort is true when the backend supports staged abort (e.g. zip://).
        // Future staging backends opt in by implementing abort() on the connection.
        const canAbort = typeof state.src.abort === 'function';
        ...
        function doAbort() {
            process.stdout.write(A.clear + sep() + '\n');
            process.stdout.write('  Abort and discard all staged changes? [y/N]: ');
            rawReadLine('').then(ans => {
                if (!ans || (ans.toLowerCase() !== 'y' && ans.toLowerCase() !== 'yes')) {
                    setMessage('Abort cancelled.');
                    render();
                    return;
                }
                state.src.abort().then(() => {
                    // Restore session file to its pre-session state.
                    // Temp files are handled by cleanup(); user files need explicit restore.
                    if (!ui.isTemp && ui.sessionFile) {
                        try {
                            if (_sessionSnapshot !== null)
                                fs.writeFileSync(ui.sessionFile, _sessionSnapshot, 'utf8');
                            else
                                fs.unlinkSync(ui.sessionFile);
                        } catch (_) {}
                    }
                    cleanup();
                });
        ...
        function cleanup() {
            ...
            if (ui.isTemp && ui.sessionFile) {
                try { fs.unlinkSync(ui.sessionFile); } catch (_) {}
            }
            resolve();
        }
        ...
        function onKey(key) {
            ...
            else if (key === 'A' && canAbort)       { doAbort();  return; }
            else if (key === '?') {
                const abortHint = canAbort ? ' A=abort' : '';
                setMessage(`k=keep d=delete l=link r=review n=none | ↑↓ files  ←→ groups | s=save S=save-as a=apply R=rescan q=quit${abortHint}`);
                render();
                return;
            }
