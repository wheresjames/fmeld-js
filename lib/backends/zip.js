...
module.exports = function zipClient(args, opts)
{
    ...
    // Walk the path components and return the portion up to and including the
    // first component with a recognised archive extension (.zip, .cbz, etc.).
    // If no such component is found, the full path is returned (existing behaviour).
    function _findArchivePath(fullPath)
    {
    ...
    function connect()
    {
        return new Promise((resolve, reject) =>
        {
            ...
            // Split args.path at the first component with a recognised archive
            // extension so that zip:///a/b.zip/c/d.txt connects to b.zip and
            // treats /c/d.txt as the internal path.
            _archivePath  = _findArchivePath(args.path);
    ...
    function _finalize()
    {
        ...
        return new Promise((resolve, reject) =>
        {
            // Pre-flight: if the staging directory was deleted externally (e.g. the
            // user removed it while interactive mode was open), abort before we
            // create any output file.  Without this guard, addDir() and countFiles()
            // both silently return with 0 files, verification passes 0===0, and
            // an empty zip is atomically swapped over the original.
            if (_stagingReady && !fs.existsSync(_stagingFiles))
                return reject(
                    `zip: staging directory missing — original archive preserved: ${_archivePath}\n` +
                    `(staging was deleted externally; no changes were written)`
                );
    ...
    function _verify()
    {
        ...
        if (!fs.existsSync(_stagingFiles))
            return Promise.reject(
                `zip: staging directory missing at verify time — original archive preserved: ${_archivePath}`
            );
    ...
    // -----------------------------------------------------------------------
    // Abort: discard all staged changes, leave original archive untouched
    // -----------------------------------------------------------------------
    ...
    function abort()
    {
        return new Promise((resolve) =>
        {
            if (!_bConnected)
                return resolve(true);
            _bConnected     = false;
            _writesOccurred = false;
            process.removeListener('exit', _onProcessExit);
            try { fs.rmSync(_stagingDir, { recursive: true, force: true }); } catch(e) {}
            Log(`zip: aborted — staging discarded, original preserved: ${_archivePath}`);
            resolve(true);
        });
    }
