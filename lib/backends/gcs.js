...
module.exports = function gcsClient(args, opts)
{
    ...
    let {Storage} = require('../setup.js').requireBackend('@google-cloud/storage', 'gs://');
        ...
        return new Promise((resolve, reject) =>
        {
            close()
                .then(r =>
                {
                    ...
                    try
                    {   const storageOpts = {projectId: credentials.project_id, credentials: credentials};
                        const emulatorHost = process.env.STORAGE_EMULATOR_HOST;
                        if (emulatorHost)
                        {   // Pass the emulator host as apiEndpoint rather than relying on
                            // STORAGE_EMULATOR_HOST.  When the SDK reads STORAGE_EMULATOR_HOST
                            // it uses it as baseUrl verbatim (no /storage/v1 appended), but
                            // upload URLs are built as ${apiEndpoint}/upload/storage/v1/...
                            // Passing apiEndpoint makes both paths consistent:
                            //   baseUrl  = apiEndpoint + '/storage/v1'
                            //   uploadUrl = apiEndpoint + '/upload/storage/v1/...'
                            storageOpts.apiEndpoint = emulatorHost;
                            delete process.env.STORAGE_EMULATOR_HOST;
                        }
                        storage = new Storage(storageOpts);
                        if (emulatorHost)
                            process.env.STORAGE_EMULATOR_HOST = emulatorHost;
                        bucket = storage.bucket(args.host);
                        bConnected = true;
                        resolve(true);
