...
{
    const ssh2 = require('../setup.js').requireBackend('ssh2', 'sftp://');
    ...
    */
    function mkDir(dir, o={})
    {
        return new Promise((resolve, reject) =>
        {
            ...
            // Recursive mkdir: create each path component in sequence
            ...
            {   if (i >= parts.length)
                ...
                {   // SSH_FX_FAILURE (code 4) typically means directory already exists
                    if (err && err.code !== 4)
                        return reject(err);
                    next();
                });
    ...
    */
    function rmDir(dir, o={})
    {
        return new Promise((resolve, reject) =>
        {
            ...
            // Recursive delete: list contents, delete each entry, then remove the directory
            sftp.readdir(dir, (err, list) =>
            {
                if (err) return reject(err);

                let tasks = (list || []).map(entry =>
                {   let full = path.posix.join(dir, entry.filename);
                    let isDir = (entry.attrs.mode & 0x4000) ? true : false;
                    return isDir ? rmDir(full, o) : rmFile(full);
                });

                Promise.all(tasks)
                    .then(() =>
                    {   sftp.rmdir(dir, (err) =>
                        {   if (err && !o.force) return reject(err);
                            return resolve(true);
                        });
                    })
                    .catch(e => { if (!o.force) return reject(e); resolve(true); });
            });
        });
    }
