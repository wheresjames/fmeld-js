...
module.exports = function fileClient(args, opts)
{
        ...
        return new Promise((resolve, reject) =>
        {
            fs.readdir(dir, (err, files) =>
            {
                ...
                for (let k in files)
                {
                    ...
                    let isFile = stats.isFile() ? true : false;
                    let isDir = stats.isDirectory() ? true : false;
                    fl.push({
                            name: v,
                            path: dir,
                            full: full,
                            isFile: isFile,
                            isDir: isDir,
                            mode: (stats.mode) ? parseInt(stats.mode) : 0,
                            size: (isFile && stats.size) ? parseInt(stats.size) : 0,
    ...
    */
    ...
    {   return new Promise((resolve, reject) =>
        {   fs.access(dir, fs.constants.F_OK, e =>
            {   if (!e)
                    return resolve(true);
                fs.mkdir(dir, {recursive: o.recursive ? true : false}, (e, p) =>
                {   if (e)
                        return reject(e);
                    return resolve(true);
                });
    ...
    */
    ...
    {   return new Promise((resolve, reject) =>
        {   fs.rmdir(dir, {recursive: o.recursive ? true : false, force: o.force ? true : false}, e =>
            {   if (e) return reject(e);
                return resolve(true);
            });
        });
